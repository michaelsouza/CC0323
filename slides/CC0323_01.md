...
![bg right:40% height:90%](figures/taylor_remainder.png)
...
$$\lim_{x\to u} \left[ f(x) - F_{\infty}(x) \right] = 0.$$

Uma vez que

$$r_k(x) = \sum_{i=k+1}^\infty \frac{f^{(i)}(u)}{i!}(x-u)^i$$

Se escrevermos $h = x - u$, teremos

$$r_k(x) = \sum_{i=k+1}^\infty \frac{f^{(i)}(u)}{i!}h^i$$

---

Agora, dividindo $r_k$ por $h^{k+1}$, obtemos

$$\frac{r_k(x)}{h^{k+1}} = \frac{f^{(k+1)}(u)}{(k+1)!} + \sum_{i=k+2}^\infty \frac{f^{(i)}(u)}{i!}h^{i-k-1}$$

Tomando $h\to 0$, chegamos à

$$\lim_{h\to 0}\frac{r_k(x)}{h^{k+1}} = \frac{f^{(k+1)}(u)}{(k+1)!},$$

ou seja, o resíduo da aproximação de $f$ por $F_k$ na vizinhança de $u$ é da ordem (mesma magnitude) de $h^{k+1}$.
