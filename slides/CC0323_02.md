...
Se $f$ é duas vezes diferenciável e $x^*$ é um mínimo de $f$, então o método de Newton converge para $x^*$ com taxa quadrática, ou seja,
...
De fato, utilizando a expansão de $(1 + x)^{-1} = 1 - x + x^2 - x^3 + \ldots$, temos
...
- O método de Newton pode ciclar 
